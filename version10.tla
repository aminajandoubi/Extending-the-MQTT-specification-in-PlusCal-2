                           ...
                           /\ LET packet == Head(network[_attaque_data[self].BrokerID]) IN
                              \/ /\ (packet.type = "SUBSCRIBE")
                                    /\ LET __attaque_data == [_attaque_data EXCEPT ![self].pkt = packet.topic] IN
                                       LET ___attaque_data == [__attaque_data EXCEPT ![self].pk = packet.QoS] IN
...
CONNECT(self) == 
                 /\ _pc[self] = "CONNECT"
                 /\ cp = any
                 /\ LET _network == send(_attaque_data[self].BrokerID, [type |-> "CONNECT", sender |-> 50, willflag |-> 1, willqos |-> 0, willretain |-> 1, session |-> 1, keepalive |-> 0, willpayload |-> "bonjour", willtopic |-> "A"]) IN
...
SUBSCRIBE(self) == 
                   /\ _pc[self] = "SUBSCRIBE"
                   /\ cp = any
                   /\ LET _network == send(_attaque_data[self].BrokerID, [type |-> "SUBSCRIBE", sender |-> 50, topic |-> _attaque_data[self].pkt, QoS |-> _attaque_data[self].pk]) IN
                      ...
                      /\ UNCHANGED << depth, cp, activeClients, sessionClients, idtopic, RetainedMessages, pk, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _stack >>
_attaque(self) == 
                      TopicSub(self) \/ CONNECT(self) \/ SUBSCRIBE(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ Lbl_4(self) \/ Lbl_5(self) \/ Lbl_6(self) \/ Lbl_7(self) \/ Lbl_8(self) \/ Lbl_9(self) \/ Lbl_10(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self)  
