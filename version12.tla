...
Init == 
    ...
    /\ _attaque_data = [self \in attaque |-> [ self |-> self, parentID |-> 0, Name|->"attaque", BrokerID|->1, INJ1|->FALSE, INJ2|->FALSE, pk|->1, pkt|->{}]]
...
CONNECT(self) == 
                 /\ _pc[self] = "CONNECT"
                 /\ cp = any
                 /\ LET _network == send(_attaque_data[self].BrokerID, [type |-> "CONNECT", sender |-> 20, willflag |-> 1, willqos |-> 0, willretain |-> 1, session |-> 1, keepalive |-> 60, willpayload |-> "bonjour", willtopic |-> "A"]) IN
                    LET __attaque_data == [_attaque_data EXCEPT ![self].INJ1 = TRUE] IN
                    /\ network' = _network
                    /\ _attaque_data' = __attaque_data
                    /\ _pc' = [_pc EXCEPT ![self] = "publishretain"]
...
publishretain(self) == 
                       /\ _pc[self] = "publishretain"
                       /\ cp = any
                       /\ LET _network == send(_attaque_data[self].BrokerID, [type |-> "PUBLISH", sender |-> 20, topic |-> "A", QoS |-> 2, retain |-> 1, payload |-> "bonjour", responsetopic |-> "A", correlationdata |-> "bonjour"]) IN
                          ...
                          /\ UNCHANGED << depth, cp, activeClients, sessionClients, idtopic, RetainedMessages, pk, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _stack >>
_attaque(self) == 
                      CONNECT(self) \/ publishretain(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ Lbl_4(self) \/ Lbl_5(self) \/ Lbl_6(self) \/ Lbl_7(self) \/ Lbl_8(self) \/ Lbl_9(self) \/ Lbl_10(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self)  
...
Temp0 == (~ \A I \in attaque : ((_attaque_data[I].INJ1 = TRUE) => (<> (_attaque_data[I].INJ2 = TRUE))))
