...
Init == 
    ...
    /\ _attaque_data = [self \in attaque |-> [ self |-> self, parentID |-> 0, Name|->"attaque", BrokerID|->1, INJ1|->FALSE]]
...
willconnect(self) == 
                     /\ _pc[self] = "willconnect"
                     /\ cp = any
                     /\ \/ /\ (Len(network[_attaque_data[self].BrokerID]) > 0)
                              /\ LET packet == Head(network[_attaque_data[self].BrokerID]) IN
                                 \/ /\ (packet.type = "SUBSCRIBE")
                                       /\ LET _network == send(_attaque_data[self].BrokerID, [type |-> "CONNECT", sender |-> 20, willflag |-> 1, willqos |-> 0, willretain |-> 1, session |-> 1, keepalive |-> 0, willpayload |-> "bonjour", willtopic |-> "A"]) IN
                                          ...
                                          \/ /\ ~((__attaque_data[self].INJ1 = TRUE))
                                                   /\ _attaque_data' = __attaque_data
                                                   /\ network' = _network
                                                   /\ _pc' = [_pc EXCEPT ![self] = "willconnect"]
                                                   /\ UNCHANGED << depth, cp, activeClients, sessionClients, idtopic, RetainedMessages, pk, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _stack >>
                                 \/ /\ ~((packet.type = "SUBSCRIBE"))
                                       /\ \/ /\ (_attaque_data[self].INJ1 = TRUE)
                                          ...
                                          \/ /\ ~((_attaque_data[self].INJ1 = TRUE))
                                                   ...
                                                   /\ UNCHANGED << depth, cp, network, activeClients, sessionClients, idtopic, RetainedMessages, pk, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
_attaque(self) == 
                      willconnect(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ Lbl_4(self) \/ Lbl_5(self) \/ Lbl_6(self) \/ Lbl_7(self) \/ Lbl_8(self) \/ Lbl_9(self) \/ Lbl_10(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self)  
...
Temp0 == (~ \A I \in attaque, I2 \in Subscriber : ((_attaque_data[I].INJ1 = TRUE) => (<> (_Subscriber_data[I2].INJ2 = TRUE))))
