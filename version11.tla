...
VARIABLES depth, cp, network, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack, _pc

vars == << depth, cp, network, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack, _pc >>
...
Init == 
    ...
    /\ _Subscriber_data = [self \in Subscriber |-> [ self |-> self, parentID |-> 0, Name|->"Subscriber", rules|->0, BrokerID|->1, SUBSUCC|->FALSE, CONNSUCC|->FALSE, PUBRECE|->FALSE, UNSUBSUCC|->FALSE]]
...
Lbl_1(self) == 
               ...
               /\ \/ /\ (({Head(_stack[self]).pkt.sender} \cap activeClients) # {})
                        /\ LET _network == send(Head(_stack[self]).pkt.sender, [type |-> "PUBCOMP", sender |-> self, topic |-> "A", QoS |-> 2]) IN
                           ...
                           /\ UNCHANGED << depth, cp, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
                  \/ /\ ~((({Head(_stack[self]).pkt.sender} \cap activeClients) # {}))
                           /\ _pc' = [_pc EXCEPT ![self] = Head(_stack[self])._pc]
                           /\ UNCHANGED << depth, cp, network, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
Lbl_2(self) == 
               /\ _pc[self] = "Lbl_2"
               /\ (cp = any \/ cp = self)
               /\ \/ /\ (Len(network[self]) > 0)
                        /\ LET packet == Head(network[self]) IN
                           \/ /\ (packet.type = "PUBREL")
                                 /\ LET _network == send(Head(_stack[self]).pkt.sender, [type |-> "PUBCOMP", sender |-> self, topic |-> "A", QoS |-> 2]) IN
                                    ...
                                    /\ _stack' = ___stack
                                    /\ _pc' = [_pc EXCEPT ![self] = Head(_stack[self])._pc]
                                    /\ UNCHANGED << depth, cp, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data >>
                           \/ /\ ~((packet.type = "PUBREL"))
                                    /\ _pc' = [_pc EXCEPT ![self] = Head(_stack[self])._pc]
                                    /\ UNCHANGED << depth, cp, network, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
Lbl_3(self) == 
...
Lbl_5(self) == 
               ...
               /\ IF TopicPool[Head(_stack[self]).pkt.topic] # {} /\ Head(_stack[self]).idS_sub = {}
                        THEN /\ LET newHead1 == Head(_stack[self]) IN
                                ...
                                /\ _pc' = [_pc EXCEPT ![self] = "Lbl_5"]
                                /\ _stack' = ___stack
                                /\ UNCHANGED << depth, cp, network, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data >>
                        ELSE
                             /\ IF Head(_stack[self]).idS_sub # {}
                                      THEN /\ LET sub == CHOOSE fech \in Head(_stack[self]).idS_sub : TRUE IN
                                              ...
                                              LET _network == send(Head(___stack[self]).sub, [type |-> "PUBLISH", sender |-> self, topic |-> Head(___stack[self]).pkt.topic, QoS |-> 2, retain |-> 0, payload |-> "bonjour"]) IN
...
Lbl_7(self) == 
               ...
               /\ IF RetainedMessages # {} /\ Head(_stack[self]).idS_sub = {}
                        THEN /\ LET newHead3 == Head(_stack[self]) IN
                                LET _newHead3 == [newHead3 EXCEPT !.idS_sub = RetainedMessages ] IN
                                LET __stack == [_stack EXCEPT ![self] = Tail(_stack[self]) ] IN
                             ...
                             /\ IF Head(_stack[self]).idS_sub # {}
                                      THEN /\ LET sub == CHOOSE fech \in Head(_stack[self]).idS_sub : TRUE IN
                                              ...
                                              /\ \/ /\ (Head(___stack[self]).sub = Head(___stack[self]).pkt.topic)
                                                    /\ LET _network == send(Head(___stack[self]).pkt.sender, [type |-> "PUBLISH", sender |-> self, topic |-> Head(___stack[self]).pkt.topic, QoS |-> Head(___stack[self]).pkt.QoS, retain |-> 0, payload |-> "bonjour"]) IN
                                                       LET __newhead == Head(___stack[self]) IN
                                                       LET ___newhead == [__newhead EXCEPT !.idS_sub = Head(___stack[self]).idS_sub \ {Head(___stack[self]).sub } ] IN
                                                       LET ____stack == [___stack EXCEPT ![self] = Tail(___stack[self]) ] IN
                                                       LET _____stack == [____stack EXCEPT ![self] = Push(____stack[self], <<___newhead>>) ] IN
...
cqos2(self) == 
               /\ _pc[self] = "cqos2"
               /\ (cp = any \/ cp = self)
               /\ LET _network == send(Head(_stack[self]).to, [type |-> "PUBLISH", sender |-> self, topic |-> "A", QoS |-> 2, retain |-> 0]) IN
                  /\ network' = _network
               ...
               \/ /\ (Head(_stack[self]).pkt.QoS = 2)
                     /\ \/ /\ (TopicPool[Head(_stack[self]).pkt.topic] # {})
                              /\ LET _network == send(Head(_stack[self]).pkt.sender, [type |-> "PUBREC", sender |-> self, topic |-> Head(_stack[self]).pkt.topic, QoS |-> 2]) IN
                                 ...
                                 /\ _pc' = [_pc EXCEPT ![self] = Head(_stack[self])._pc]
                                 /\ UNCHANGED << depth, cp, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
                        \/ /\ ~((TopicPool[Head(_stack[self]).pkt.topic] # {}))
                                 /\ _pc' = [_pc EXCEPT ![self] = Head(_stack[self])._pc]
                                 /\ UNCHANGED << depth, cp, network, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
Lbl_9(self) == 
               ...
               /\ IF TopicPool[Head(_stack[self]).pkt.topic] # {} /\ Head(_stack[self]).idS_sub = {}
                        THEN /\ LET newHead5 == Head(_stack[self]) IN
                                LET _newHead5 == [newHead5 EXCEPT !.idS_sub = TopicPool[Head(_stack[self]).pkt.topic] ] IN
                                LET __stack == [_stack EXCEPT ![self] = Tail(_stack[self]) ] IN
                        ...
                        ELSE
                             /\ IF Head(_stack[self]).idS_sub # {}
                                      THEN /\ LET sub == CHOOSE fech \in Head(_stack[self]).idS_sub : TRUE IN
                                              ...
                                              LET _network == send(Head(___stack[self]).sub, [type |-> "PUBLISH", sender |-> self, topic |-> "A", QoS |-> 0, retain |-> 1, payload |-> "bonjour"]) IN
                                              LET __newhead == Head(___stack[self]) IN
                                              LET ___newhead == [__newhead EXCEPT !.idS_sub = Head(___stack[self]).idS_sub \ {Head(___stack[self]).sub } ] IN
                                              LET ____stack == [___stack EXCEPT ![self] = Tail(___stack[self]) ] IN
                                              LET _____stack == [____stack EXCEPT ![self] = Push(____stack[self], <<___newhead>>) ] IN
                         ...
                         ELSE
                              /\ IF Head(_stack[self]).idS_sub # {}
                                       THEN /\ LET sub == CHOOSE fech \in Head(_stack[self]).idS_sub : TRUE IN
                                               ...
                                               LET _network == send(Head(___stack[self]).sub, [type |-> "PUBLISH", sender |-> self, topic |-> Head(___stack[self]).pkt.topic, QoS |-> 1, retain |-> 0, payload |-> "bonjour"]) IN
                                               LET __newhead == Head(___stack[self]) IN
                                               LET ___newhead == [__newhead EXCEPT !.idS_sub = Head(___stack[self]).idS_sub \ {Head(___stack[self]).sub } ] IN
                                               LET ____stack == [___stack EXCEPT ![self] = Tail(___stack[self]) ] IN
                                               LET _____stack == [____stack EXCEPT ![self] = Push(____stack[self], <<___newhead>>) ] IN
                                               /\ IF Head(_____stack[self]).idS_sub # {}
                                                     THEN    /\ _pc' = [_pc EXCEPT ![self] = "Lbl_10"]
                                                             /\ _stack' = _____stack
                                                             /\ network' = _network
                                                             /\ UNCHANGED << depth, cp, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data >>
                                                     ELSE
                            ...
                            \/ /\ (packet.type = "CONNECT")
                                  /\ \/ /\ (packet.session = 1)
                                           /\ LET _sessionClients == (sessionClients \cup {packet.sender}) IN
                                              ...
                                              /\ UNCHANGED << depth, cp, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
                                     \/ /\ ~((packet.session = 1))
                                           /\ LET _activeClients == (activeClients \cup {packet.sender}) IN
                                              LET _network == send(packet.sender, [type |-> "CONNACK", sender |-> self]) IN
                                              LET __network == [_network EXCEPT ![self] = Tail(_network[self])] IN
                                              ...
                                              /\ network' = __network
                                              /\ _pc' = [_pc EXCEPT ![self] = "listen"]
                                              /\ UNCHANGED << depth, cp, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
                            \/ /\ (packet.type = "SUBSCRIBE")
                                  /\ \/ /\ (({packet.sender} \cap activeClients) # {})
                                           /\ LET _TopicPool == [TopicPool EXCEPT ![packet.topic] = (TopicPool[packet.topic] \cup {packet.sender})] IN
                                              ...
                                              LET __stack == [_stack EXCEPT ![self] = Push(_stack[self], <<[pkt|->packet, sub|->0, idS_sub|->{}, _pc |->"Lbl_11"]>>)] IN
                                              /\ _pc' = [_pc EXCEPT ![self] = "Lbl_6"]
                                              /\ TopicPool' = _TopicPool
                                              /\ network' = _network
                                              /\ _stack' = __stack
                                              /\ UNCHANGED << depth, cp, activeClients, sessionClients, RetainedMessages, Topics, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data >>
                                     \/ /\ ~((({packet.sender} \cap activeClients) # {}))
                                              /\ _pc' = [_pc EXCEPT ![self] = "listen"]
                                              /\ UNCHANGED << depth, cp, network, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
                            \/ /\ (packet.type = "PUBLISH")
                                  /\ \/ /\ (packet.retain = 1)
                                           /\ LET _RetainedMessages == (RetainedMessages \cup {packet.topic}) IN
                                              LET __stack == [_stack EXCEPT ![self] = Push(_stack[self], <<[pkt|->packet, sub|->0, idS_sub|->{}, _pc |->"Lbl_12"]>>)] IN
                                              /\ _pc' = [_pc EXCEPT ![self] = "l1"]
                                              /\ RetainedMessages' = _RetainedMessages
                                              /\ _stack' = __stack
                                              /\ UNCHANGED << depth, cp, network, activeClients, sessionClients, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data >>
                                     \/ /\ ~((packet.retain = 1))
                            ...
                            \/ /\ (packet.type = "PUBREL")
                                  /\ \/ /\ (({packet.sender} \cap _Broker_data[self].wait_REL) # {})
                                           /\ LET __Broker_data == [_Broker_data EXCEPT ![self].wait_REL = (_Broker_data[self].wait_REL \ {packet.sender})] IN
                                              ...
                                              /\ network' = _network
                                              /\ _stack' = __stack
                                              /\ UNCHANGED << depth, cp, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Publisher_data, _Subscriber_data, _attaque_data >>
                                     \/ /\ ~((({packet.sender} \cap _Broker_data[self].wait_REL) # {}))
                            ...
                            \/ /\ (packet.type = "UNSUBSCRIBE")
                                  /\ \/ /\ (({packet.sender} \cap activeClients) # {})
                                           /\ LET _TopicPool == [TopicPool EXCEPT ![packet.topic] = (TopicPool[packet.topic] \ {packet.sender})] IN
                                              ...
                                              /\ network' = __network
                                              /\ _pc' = [_pc EXCEPT ![self] = "listen"]
                                              /\ UNCHANGED << depth, cp, activeClients, sessionClients, RetainedMessages, Topics, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
                                     \/ /\ ~((({packet.sender} \cap activeClients) # {}))
                                              /\ _pc' = [_pc EXCEPT ![self] = "listen"]
                                              /\ UNCHANGED << depth, cp, network, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
                            \/ /\ (packet.type = "PINGREQ")
                                  /\ LET __stack == [_stack EXCEPT ![self] = Push(_stack[self], <<[pkt|->packet, _pc |->"Lbl_16"]>>)] IN
...
Lbl_16(self) == 
                ...
                /\ LET _network == [network EXCEPT ![self] = Tail(network[self])] IN
                   ...
                   /\ UNCHANGED << depth, cp, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
_Broker(self) == 
                     listen(self) \/ Lbl_11(self) \/ Lbl_12(self) \/ Lbl_13(self) \/ Lbl_14(self) \/ Lbl_15(self) \/ Lbl_16(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ Lbl_4(self) \/ Lbl_5(self) \/ Lbl_6(self) \/ Lbl_7(self) \/ Lbl_8(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self) \/ l1(self) \/ Lbl_9(self) \/ Lbl_10(self)  
PubStart(self) == 
                  /\ _pc[self] = "PubStart"
                  /\ cp = any
                  /\ LET _network == send(_Publisher_data[self].BrokerID, [type |-> "CONNECT", sender |-> self, willflag |-> 1, willqos |-> 0, willretain |-> 1, session |-> 1, keepalive |-> 0, willpayload |-> "bonjour", willtopic |-> "A"]) IN
                     /\ network' = _network
                              ...
                              \/ /\ (packet.type = "CONNACK")
                                    /\ LET __Publisher_data == [_Publisher_data EXCEPT ![self].CONNSUCC = TRUE] IN
                                       ...
                                       \/ /\ (__Publisher_data[self].CONNSUCC = TRUE)
                                                ...
                                                /\ network' = _network
                                                /\ UNCHANGED << depth, cp, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Subscriber_data, _attaque_data, _stack >>
                                       \/ /\ ~((__Publisher_data[self].CONNSUCC = TRUE))
                                                /\ _Publisher_data' = __Publisher_data
                                                /\ network' = _network
...
lbl(self) == 
             /\ _pc[self] = "lbl"
             /\ cp = any
             /\ LET _network == send(_Publisher_data[self].BrokerID, [type |-> "PUBLISH", sender |-> self, topic |-> "A", QoS |-> 0, retain |-> 0, payload |-> "bonjour"]) IN
...
tryPUB1(self) == 
                 /\ _pc[self] = "tryPUB1"
                 /\ cp = any
                 /\ LET _network == send(_Publisher_data[self].BrokerID, [type |-> "PUBLISH", sender |-> self, topic |-> "A", QoS |-> 1, retain |-> 0, payload |-> "bonjour"]) IN
                    /\ network' = _network
...
tryPUB2(self) == 
                 /\ _pc[self] = "tryPUB2"
                 /\ cp = any
                 /\ LET __stack == [_stack EXCEPT ![self] = Push(_stack[self], <<[to|->_Publisher_data[self].BrokerID, PUBSUCC|->FALSE, _pc |->"Lbl_17"]>>)] IN
                    /\ _pc' = [_pc EXCEPT ![self] = "cqos2"]
                    /\ _stack' = __stack
                    /\ UNCHANGED << depth, cp, network, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data >>
_Publisher(self) == 
                        PubStart(self) \/ waitCONN(self) \/ tryPUBLISH(self) \/ lbl(self) \/ tryPUB1(self) \/ waitPUB1(self) \/ Lbl_17(self) \/ tryPUB2(self) \/ Lbl_17(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ Lbl_4(self) \/ Lbl_5(self) \/ Lbl_6(self) \/ Lbl_7(self) \/ Lbl_8(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self)  
Sub_start(self) == 
                   /\ _pc[self] = "Sub_start"
                   /\ cp = any
                   /\ LET _network == send(_Subscriber_data[self].BrokerID, [type |-> "CONNECT", sender |-> self, willflag |-> 1, willqos |-> 0, willretain |-> 1, session |-> 1, keepalive |-> 0, willpayload |-> "bonjour", willtopic |-> "A"]) IN
                      /\ network' = _network
                                  ...
                                  \/ /\ (packet.type = "CONNACK")
                                        /\ LET __Subscriber_data == [_Subscriber_data EXCEPT ![self].CONNSUCC = TRUE] IN
                                           ...
                                           \/ /\ (__Subscriber_data[self].CONNSUCC = TRUE)
                                                    ...
                                                    /\ network' = _network
                                                    /\ UNCHANGED << depth, cp, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _attaque_data, _stack >>
                                           \/ /\ ~((__Subscriber_data[self].CONNSUCC = TRUE))
                                                    /\ _Subscriber_data' = __Subscriber_data
                                                    /\ network' = _network
                                                        ...
                                                        /\ LET __network == send(packet.sender, [type |-> "PUBACK", sender |-> self, topic |-> "A", QoS |-> 1]) IN
                                                           ...
                                                           \/ /\ (packet.QoS = 2)
                                                                 /\ LET ____network == send(packet.sender, [type |-> "PUBREC", sender |-> self, topic |-> "A", QoS |-> 2]) IN
                                                                    ...
                                                                    LET __stack == [_stack EXCEPT ![self] = Push(_stack[self], <<[pkt|->packet, PUBRECE|->FALSE, _pc |->"Lbl_18"]>>)] IN
                                                                    /\ _pc' = [_pc EXCEPT ![self] = "Lbl_2"]
                                                                    /\ _Subscriber_data' = ___Subscriber_data
                                                                    /\ network' = _____network
                                                                    /\ _stack' = __stack
                                                                    /\ UNCHANGED << depth, cp, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _attaque_data >>
                                                           \/ /\ ~((packet.QoS = 2))
                                                  ...
                                                  \/ /\ ~((packet.QoS = 1))
                                                        /\ \/ /\ (packet.QoS = 2)
                                                                 /\ LET __network == send(packet.sender, [type |-> "PUBREC", sender |-> self, topic |-> "A", QoS |-> 2]) IN
                                                                    LET ___network == [__network EXCEPT ![self] = Tail(__network[self])] IN
                                                                    LET __stack == [_stack EXCEPT ![self] = Push(_stack[self], <<[pkt|->packet, PUBRECE|->FALSE, _pc |->"Lbl_19"]>>)] IN
                                                                    /\ _pc' = [_pc EXCEPT ![self] = "Lbl_2"]
                                                                    /\ _Subscriber_data' = __Subscriber_data
                                                                    /\ network' = ___network
                                                                    /\ _stack' = __stack
                                                        ...
                                                        /\ LET _network == send(packet.sender, [type |-> "PUBACK", sender |-> self, topic |-> "A", QoS |-> 1]) IN
                                                           ...
                                                           \/ /\ (packet.QoS = 2)
                                                                 /\ LET ___network == send(packet.sender, [type |-> "PUBREC", sender |-> self, topic |-> "A", QoS |-> 2]) IN
                                                                    ...
                                                                    LET __stack == [_stack EXCEPT ![self] = Push(_stack[self], <<[pkt|->packet, PUBRECE|->FALSE, _pc |->"Lbl_20"]>>)] IN
                                                                    /\ _pc' = [_pc EXCEPT ![self] = "Lbl_2"]
                                                                    /\ network' = ____network
                                                                    /\ _Subscriber_data' = __Subscriber_data
                                                                    /\ _stack' = __stack
                                                  ...
                                                  \/ /\ ~((packet.QoS = 1))
                                                        /\ \/ /\ (packet.QoS = 2)
                                                                 /\ LET _network == send(packet.sender, [type |-> "PUBREC", sender |-> self, topic |-> "A", QoS |-> 2]) IN
                                                                    LET __network == [_network EXCEPT ![self] = Tail(_network[self])] IN
                                                                    LET __stack == [_stack EXCEPT ![self] = Push(_stack[self], <<[pkt|->packet, PUBRECE|->FALSE, _pc |->"Lbl_21"]>>)] IN
                                                                    /\ _pc' = [_pc EXCEPT ![self] = "Lbl_2"]
                                                                    /\ network' = __network
                                                                    /\ _stack' = __stack
                                                                    /\ UNCHANGED << depth, cp, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data >>
...
Lbl_18(self) == 
                ...
                /\ LET __Subscriber_data == [_Subscriber_data EXCEPT ![self].PUBRECE = TRUE] IN
                   \/ /\ (__Subscriber_data[self].PUBRECE = TRUE)
                            /\ _pc' = [_pc EXCEPT ![self] = "unSub_subscription"]
                            /\ _Subscriber_data' = __Subscriber_data
                            /\ UNCHANGED << depth, cp, network, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _attaque_data, _stack >>
                   \/ /\ ~((__Subscriber_data[self].PUBRECE = TRUE))
                            /\ _Subscriber_data' = __Subscriber_data
                            /\ _pc' = [_pc EXCEPT ![self] = "Sub_listen"]
                            /\ UNCHANGED << depth, cp, network, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _attaque_data, _stack >>
...
Disconnectsub(self) == 
                       ...
                       /\ LET _network == send(_Subscriber_data[self].BrokerID, [type |-> "DISCONNECT", sender |-> self]) IN
                          ...
                          /\ UNCHANGED << depth, cp, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
_Subscriber(self) == 
                         Sub_start(self) \/ Sub_waitCONN(self) \/ Sub_subscription(self) \/ sub0(self) \/ sub1(self) \/ sub2(self) \/ Sub_waitsubscription(self) \/ Sub_listen(self) \/ Lbl_18(self) \/ Lbl_19(self) \/ Lbl_20(self) \/ Lbl_21(self) \/ unSub_subscription(self) \/ unSub_waitsubscription(self) \/ pingreq(self) \/ Disconnectsub(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ Lbl_4(self) \/ Lbl_5(self) \/ Lbl_6(self) \/ Lbl_7(self) \/ Lbl_8(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self)  
CONNECT(self) == 
                 /\ _pc[self] = "CONNECT"
                 /\ cp = any
                          ...
                          /\ LET packet == Head(network[_attaque_data[self].BrokerID]) IN
                             \/ /\ (packet.type = "CONNECT")
                                   /\ LET __attaque_data == [_attaque_data EXCEPT ![self].pk = packet.sender] IN
                                      ...
                                      LET _network == send(___attaque_data[self].BrokerID, [type |-> "CONNECT", sender |-> 3, willflag |-> 1, willqos |-> 0, willretain |-> 1, session |-> 1, keepalive |-> 0, willpayload |-> "connecting", willtopic |-> "A"]) IN
                                      \/ /\ (___attaque_data[self].OBS = TRUE)
                                               /\ _pc' = [_pc EXCEPT ![self] = "CONNACK"]
                                               /\ _attaque_data' = ___attaque_data
                                               /\ network' = _network
                                               /\ UNCHANGED << depth, cp, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _stack >>
                                      \/ /\ ~((___attaque_data[self].OBS = TRUE))
                                               /\ _attaque_data' = ___attaque_data
                                               /\ network' = _network
                             ...
                             \/ /\ ~((packet1.type = "CONNACK"))
                                   /\ \/ /\ (_attaque_data[self].INJ = TRUE)
                                      ...
                                      \/ /\ ~((_attaque_data[self].INJ = TRUE))
                                               ...
                                               /\ UNCHANGED << depth, cp, network, activeClients, sessionClients, RetainedMessages, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
_attaque(self) == 
                      CONNECT(self) \/ CONNACK(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ Lbl_4(self) \/ Lbl_5(self) \/ Lbl_6(self) \/ Lbl_7(self) \/ Lbl_8(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self)  
