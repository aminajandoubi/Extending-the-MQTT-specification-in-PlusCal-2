...
Init == 
    ...
    /\ _pc = [self \in ProcSet |-> CASE self \in Broker -> "listen"
                         ...
                         []self \in attaque -> "subcorrelation"]
...
subcorrelation(self) == 
                        /\ _pc[self] = "subcorrelation"
                        /\ cp = any
                        /\ \/ /\ (Len(network[_attaque_data[self].BrokerID]) > 0)
                                 /\ LET packet == Head(network[_attaque_data[self].BrokerID]) IN
                                    \/ /\ (packet.type = "SUBSCRIBE")
                                          /\ LET _idtopic == packet.sender IN
                                             LET __attaque_data == [_attaque_data EXCEPT ![self].OBS = TRUE] IN
                                             LET ___attaque_data == [__attaque_data EXCEPT ![self].pkt = packet.topic] IN
                                             \/ /\ (___attaque_data[self].OBS = TRUE)
                                                      /\ _pc' = [_pc EXCEPT ![self] = "connectcorrelation"]
                                                      /\ idtopic' = _idtopic
                                                      /\ _attaque_data' = ___attaque_data
                                                      /\ UNCHANGED << depth, cp, network, activeClients, sessionClients, RetainedMessages, pk, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _stack >>
                                             \/ /\ ~((___attaque_data[self].OBS = TRUE))
                                                      /\ idtopic' = _idtopic
                                                      /\ _attaque_data' = ___attaque_data
                                                      /\ _pc' = [_pc EXCEPT ![self] = "subcorrelation"]
                                                      /\ UNCHANGED << depth, cp, network, activeClients, sessionClients, RetainedMessages, pk, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _stack >>
                                    \/ /\ ~((packet.type = "SUBSCRIBE"))
                                          /\ \/ /\ (_attaque_data[self].OBS = TRUE)
                                             ...
                                             \/ /\ ~((_attaque_data[self].OBS = TRUE))
                                                      /\ _pc' = [_pc EXCEPT ![self] = "subcorrelation"]
                                                      /\ UNCHANGED << depth, cp, network, activeClients, sessionClients, idtopic, RetainedMessages, pk, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
connectcorrelation(self) == 
                            /\ _pc[self] = "connectcorrelation"
                            /\ cp = any
                            /\ LET _network == send(_attaque_data[self].BrokerID, [type |-> "CONNECT", sender |-> 20, willflag |-> 1, willqos |-> 0, willretain |-> 1, session |-> 0, keepalive |-> 60, willpayload |-> "bonjour", willtopic |-> "A"]) IN
                               LET __attaque_data == [_attaque_data EXCEPT ![self].INJ1 = TRUE] IN
                               /\ network' = _network
                               /\ _attaque_data' = __attaque_data
                               /\ _pc' = [_pc EXCEPT ![self] = "publishcorrelation"]
...
publishcorrelation(self) == 
                            /\ _pc[self] = "publishcorrelation"
                            /\ cp = any
                            /\ LET _network == send(_attaque_data[self].BrokerID, [type |-> "PUBLISH", sender |-> 20, topic |-> _attaque_data[self].pkt, QoS |-> 0, retain |-> 0, payload |-> "bonjour", responsetopic |-> "test", correlationdata |-> "test"]) IN
                               ...
                               /\ UNCHANGED << depth, cp, activeClients, sessionClients, idtopic, RetainedMessages, pk, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _stack >>
_attaque(self) == 
                      subcorrelation(self) \/ connectcorrelation(self) \/ publishcorrelation(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ Lbl_4(self) \/ Lbl_5(self) \/ Lbl_6(self) \/ Lbl_7(self) \/ Lbl_8(self) \/ Lbl_9(self) \/ Lbl_10(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self)  
