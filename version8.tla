                          ...
                          /\ LET packet == Head(network[_attaque_data[self].BrokerID]) IN
                             \/ /\ (packet.type = "PUBCOMP")
                                   /\ LET __attaque_data == [_attaque_data EXCEPT ![self].OBS = TRUE] IN
                                      LET _network == send(__attaque_data[self].BrokerID, [type |-> "PUBCOMP", sender |-> packet.sender, topic |-> packet.topic, QoS |-> packet.QoS]) IN
                             ...
                             \/ /\ ~((packet.type = "PUBCOMP"))
                                   /\ \/ /\ (_attaque_data[self].INJ = TRUE)
                                      ...
                                      \/ /\ ~((_attaque_data[self].INJ = TRUE))
                                               ...
                                               /\ UNCHANGED << depth, cp, network, activeClients, sessionClients, idtopic, RetainedMessages, pk, Topics, TopicPool, _Broker_data, _Publisher_data, _Subscriber_data, _attaque_data, _stack >>
_attaque(self) == 
                      PUBCOMP(self) \/ Lbl_1(self) \/ Lbl_2(self) \/ Lbl_3(self) \/ Lbl_4(self) \/ Lbl_5(self) \/ Lbl_6(self) \/ Lbl_7(self) \/ Lbl_8(self) \/ Lbl_9(self) \/ Lbl_10(self) \/ cqos2(self) \/ waitPUB2(self) \/ waitPUBCOMP2(self)  
